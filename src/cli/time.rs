//! Parsing and rendering of the time specs the CLI accepts.
//!
//! Four input grammars converge on one `DateTime<Utc>`: relative (`5m`),
//! RFC 3339, the offset-bearing form `unf log` prints, and a bare local
//! time. Rendering is the inverse, so a timestamp read out of `unf log`
//! parses back to the instant it came from.
//!
//! Every function here is pure except `parse_time_spec`, which reads the
//! clock once and passes it to `parse_relative` — the seam the tests drive
...
fn invalid_time(spec: &str) -> crate::error::UnfError {
    crate::error::UnfError::InvalidTime(format!(
        "not a time: \"{spec}\"\n  \
         Try 5m, 2h, 3d, \"2026-08-25 21:10:03 -0600\" (as `unf log` prints it),\n  \
...
mod tests {
    ...
    fn invalid_time_message_is_two_lines_and_teaches_every_grammar() {
        ...
        assert_eq!(lines.len(), 3, "message was: {msg}");
        ...
        // Every accepted grammar, and the shell quoting on the two that
        // contain a space. The offset form is the one `unf log` prints, so
        // leaving it out told the user their own output was invalid input.
        let hint = lines[1..].join(" ");
        assert!(hint.contains("5m"));
        assert!(hint.contains("\"2026-08-25 21:10:03 -0600\""));
        assert!(hint.contains("\"2026-08-25 21:10:03\" (local)"));
        assert!(hint.contains("2026-08-25T21:10:03Z (UTC)"));
    }
