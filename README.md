...
```bash
curl -fsSLO https://downloads.unfudged.io/releases/v0.21.3/unf_0.21.3_amd64.deb
sudo dpkg -i unf_0.21.3_amd64.deb
```

For ARM64:
```bash
curl -fsSLO https://downloads.unfudged.io/releases/v0.21.3/unf_0.21.3_arm64.deb
sudo dpkg -i unf_0.21.3_arm64.deb
```
...
| `unf prune --all-projects` | Same, across every registered project. Cannot be undone |
| `unf prune --all-projects --yes` | Confirm a prune that would leave a project with no snapshots at all |
