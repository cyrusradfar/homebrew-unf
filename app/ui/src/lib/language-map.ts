...
const FUNCTION_PATTERNS: Record<string, RegExp> = {
  typescript: /^\s*(export\s+)?(default\s+)?(async\s+)?(function\*?\s+\w+|class\s+\w+|const\s+\w+\s*=\s*(async\s+)?[\(]|interface\s+\w+|type\s+\w+|enum\s+\w+)/,
  javascript: /^\s*(export\s+)?(default\s+)?(async\s+)?(function\*?\s+\w+|class\s+\w+|const\s+\w+\s*=\s*(async\s+)?[\(])/,
  ...
  csharp: /^\s*(public|private|protected|internal)?\s*(static\s+)?(class|interface|enum|void|int|string|bool|async)\s+\w+/,
...
export function extractFunctionName(line: string, lang: string): string | null {
  ...
  const nameMatch = match[0].match(/(?:fn|def|function\*?|class|struct|enum|trait|impl|mod|type|interface|module|object|protocol|macro_rules!)\s+(\w+)/);
