<script lang="ts">
  ...
  let sessions = $derived.by((): Session[] => {
    ...
    const active = buckets
      .map((b, i) => ({ idx: i, bucket: b }))
      .filter((x) => x.bucket.count > 0);
    ...
    const bucketMs =
      new Date(buckets[0].end).getTime() - new Date(buckets[0].start).getTime();
    ...
    for (let i = 1; i < active.length; i++) {
      ...
      const curr = active[i];
      const gap =
        new Date(buckets[curr.idx].start).getTime() -
        new Date(buckets[prev.idx].end).getTime();
